...
function MembershipCardDark({ config, selectedSurface, onSurfaceChange, onSelect }) {
  const Icon = config.icon;
  const isPopular = config.popular;
  
  // Déterminer les infos à afficher en fonction de la sélection
  const displayInfo = config.hasSelector && selectedSurface 
    ? config.selectorOptions.find(opt => opt.id === selectedSurface)
    : null;
  
  const currentPrice = displayInfo ? displayInfo.price : config.price;
  const currentDescription = displayInfo ? displayInfo.description : config.description;
  const currentBenefits = displayInfo ? displayInfo.benefits : config.benefits;
  const currentUrl = displayInfo ? displayInfo.helloAssoUrl : config.helloAssoUrl;
  ...
  return (
    ...
    >
      ...
      <CardContent className="flex-1 flex flex-col pt-0">
        {/* Sélecteur de surface pour commerçants */}
        {config.hasSelector && (
          <div className="mb-4 p-3 bg-slate-700/50 rounded-lg border border-slate-600">
            ...
            <Select value={selectedSurface || ''} onValueChange={onSurfaceChange}>
              <SelectTrigger className="bg-slate-800 border-slate-600 text-white">
                <SelectValue placeholder="Moins de 100m² (50€)" />
              </SelectTrigger>
              <SelectContent className="bg-slate-800 border-slate-600">
                <SelectItem value="base" className="text-white hover:bg-slate-700">
                  Moins de 100m² — 50€
                </SelectItem>
        ...
        <div className="border-t border-slate-700 pt-4 mb-4 flex-1">
          <ul className="space-y-2">
            {currentBenefits.slice(0, 4).map((benefit, idx) => (
              <li key={idx} className="flex items-start text-sm">
                <CheckCircle2 className="h-4 w-4 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-gray-300">{benefit}</span>
              </li>
            ))}
            {currentBenefits.length > 4 && (
              <li className="text-xs text-gray-500 pl-6">
                + {currentBenefits.length - 4} autres avantages
              </li>
            )}
          </ul>
        </div>
...
function MembershipCardLight({ config, selectedSurface, onSurfaceChange }) {
  const Icon = config.icon;
  const isPopular = config.popular;
...
export default function MembershipSelector({ variant = 'dark', onSelect, showTitle = true }) {
  const [commercantsSurface, setCommercantsSurface] = useState('');
  ...
  return (
    <div className={variant === 'dark' ? 'space-y-8' : 'space-y-6'}>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {Object.entries(MEMBERSHIP_CONFIG).map(([key, config]) => (
          <CardComponent
            key={key}
            config={config}
            selectedSurface={key === 'commercants' ? commercantsSurface : undefined}
            onSurfaceChange={key === 'commercants' ? setCommercantsSurface : undefined}
            onSelect={onSelect}
          />
        ))}
      </div>
